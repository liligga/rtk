...
pub const RULES: &[RtkRule] = &[
    ...
    RtkRule {
        pattern: r"^((uv\s+run\s+(-q\s+)?(python[0-9.]*\s+-m\s+)?)|(python[0-9.]*\s+-m\s+)?)pytest(\s|$)",
        rtk_cmd: "rtk pytest",
        rewrite_prefixes: &[
            "uv run -q python3 -m pytest",
    ...
    RtkRule {
        pattern: r"^((uv\s+run\s+(-q\s+)?python[0-9.]*\s+-m\s+)|(python[0-9.]*\s+-m\s+))py_compile(\s|$)",
        rtk_cmd: "rtk py-compile",
        rewrite_prefixes: &[
            "uv run -q python3 -m py_compile",
            "uv run -q python -m py_compile",
